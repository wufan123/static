...
new Vue({
  ...
  computed: {
    rolesHeroList: function () {
      return this.heroList.filter((item) => {
        ...
        return (
          rolesStr.indexOf(this.roles) >= 0 &&
          item.keywords.indexOf(this.searchKey ? this.searchKey : "") >= 0
        );
      });
    },
  },
  methods: {
    ...
    changeGrade: function (skill, num) {
      ...
      if (num == 1) {
        skill.grade =
          newGrade >= skill.cooldown.length - 1
            ? skill.cooldown.length - 1
            : newGrade;
      } else {
    ...
    clickHeroSkillIcon: function (line, skill) {
      if (skill.canUse == 0) {
        skill.canUse = this.getCDAfterSpeed(
          skill.cooldown ? skill.cooldown[skill.grade] : 0,
          line.heroDetail.spellSpeed
        );
      } else {
    ...
    getHeroList: async function () {
      ...
      this.heroList = data.hero.map((item) => {
        return {
          icon: `//game.gtimg.cn/images/lol/act/img/champion/${item.alias}.png`,
          ...item,
        };
      });
    },
    ...
    clickIcon(line, skill) {
      if (line.mode) {
        if (skill.canUse == 0) {
          skill.canUse = this.getCDAfterSpeed(
            skill.cd,
            line.skillSpeed.length ? line.skillSpeed.reduce((a, b) => a + b) : 0
          );
        } else {
          skill.canUse = skill.canUse - 10 >= 0 ? skill.canUse - 10 : 0;
        }
